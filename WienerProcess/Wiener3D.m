...
%Third Brownian/Wiener process
zWienerSteps=mu*tDelta+sigma*sqrt(tDelta)...
    .*randn([numb_t,numbPaths]); %normally distributed steps
zWiener=cumsum(zWienerSteps); %find Wiener/Brownian process values
zWiener=zWiener-zWiener(1,:); %set initial value to zero 
%%%END Create Brownian/Wiener processes END%%%

%%%START Plotting START%%%
figure;
plot3(xWiener,yWiener,zWiener);
grid;
title('Three-dimensional Wiener (or Brownian motion) process');
xlabel("x");
ylabel("y");
ylabel("z");
%%%END Plotting END%%%
