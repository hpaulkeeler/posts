...
xRand=np.random.uniform(xMin,xMax,simNumb); #random intial values
yRand=np.random.uniform(yMin,yMax,simNumb); #random intial values

probCurrent=fun_p(xRand,yRand); #current transition probabilities

for jj in range(numbSteps):
    zxRand= xRand +sigma*np.random.normal(0,1,simNumb);#take a (normally distributed) random step        
    zyRand= yRand +sigma*np.random.normal(0,1,simNumb);#take a (normally distributed) random step        
...
xRand= np.reshape(xRand,simNumb);
yRand= np.reshape(yRand,simNumb);
